...
const FacebookIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
    <svg {...props} fill="currentColor" viewBox="-5 0 20 20" xmlns="http://www.w3.org/2000/svg">
...
export const sierraChartTemplates = [
  ...
  if (a.fileName.startsWith('hacker') && !b.fileName.startsWith('hacker')) return 1;
  if (!a.fileName.startsWith('hacker') && b.fileName.startsWith('hacker')) return -1;
...
export const mt5Templates = [
    ...
    const parts = template.description.split(', Price LKR ');
    const name = parts[0];
    const price = parts[1];
    const isFree = price.toLowerCase().includes('free');
    const downloadUrl = isFree ? freeTemplateMapping[name] : undefined;
